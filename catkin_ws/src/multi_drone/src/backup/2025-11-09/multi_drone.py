...
class DroneUnit:
    def __init__(self, ns):
        ...
        self.pub_depth_color = rospy.Publisher(f'/{ns}/depth_map_color', ROSImage, queue_size=10)  # 给DRL Agent
        self.pub_depth_mono = rospy.Publisher(f'/{ns}/depth_map_mono', ROSImage, queue_size=10)    # 给ORB-SLAM3
            ...
            if self.image_ready and self.current_image is not None:
                try:
                    ...
                    if depth_map is not None:
                        if color_msg:
                            self.pub_depth_color.publish(color_msg)
                        
                        if mono_msg:
                            self.pub_depth_mono.publish(mono_msg)

                        # DRL决策使用彩色深度图（保持现有逻辑）
                        vel_cmd = self.drl_agent.make_decision(depth_map, '0')
                        # 添加高度控制到速度指令
